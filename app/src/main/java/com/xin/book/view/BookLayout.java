...
public class BookLayout extends FrameLayout {
	...
	private final int clickCornerLen = 100 * 100; // 50dip
...
//	private Listener mListener;
    ...
    public void setViewBitmap(Bitmap bitmap, Bitmap coverBitmap) {
        mPageAdapter.setBitmap(bitmap);
        mPageAdapter.setCoverBitmap(coverBitmap);
    }
	...
	OnTouchListener touchListener = new OnTouchListener() {
		public boolean onTouch(View v, MotionEvent event) {
            ...
            else if(mSelectCorner == Corner.None && indexPage != 0) {
                if (event.getAction() == MotionEvent.ACTION_DOWN) {
                    Log.d(LOG_TAG, "onTouch " + " x: " + event.getX() + " y: " + event.getY() + " mState:" + mState);
                    mBookView.startx = event.getX();
                    mBookView.starty = event.getY();
                }
                else if (event.getAction() == MotionEvent.ACTION_MOVE) {
                    mBookView.endx = event.getX();
                    mBookView.endy = event.getY();

                    mBookView.mCanvas.drawLine(mBookView.startx, mBookView.starty, mBookView.endx, mBookView.endy, mBookView.mPaint);
                    //重绘图像
                    mBookView.update();
                    //重新设置起始坐标
                    mBookView.startx = mBookView.endx;
                    mBookView.starty = mBookView.endy;
                }
		...
		public void update() {
			...
			try {
				synchronized (surfaceHolder) {
                    ...
                    mCanvas.drawBitmap(tmpBmp, 0, 0, mPaint);
                    canvas.drawBitmap(tmpBmp, 0, 0, mPaint);
                    //
				}
			} catch (Exception e) {
				e.printStackTrace(); 
